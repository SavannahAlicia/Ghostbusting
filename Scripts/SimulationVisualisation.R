...
bias_dat <- function(scr_fit,
  ...
  scr_coefs <- data.frame(t(sapply(scr_fit, function(x) coef(x)[,1]))) %>% 
    mutate(mod = 'scr', trueD = true_D,
           d1 = true_d1, true_lambda0 = true_lambda0,
  ...
  mod_bias <- rbind(scr_coefs, scr2_coefs) %>%
    ...
    summarise(EN_Bias = mean(EN_b), 
              ...
              lambda0_Bias = mean(lambda0_b),  
              lambda0_lcl = lambda0_Bias - 1.96 * sd(lambda0_b)/sqrt(n()),
              lambda0_ucl = lambda0_Bias + 1.96 * sd(lambda0_b)/sqrt(n()),
...
all_sims = bind_rows(lp_ld_dat,
                     lp_hd_dat,
                     hp_ld_dat,
                     hp_hd_dat) 

all_sims$sim_name = factor(all_sims$sim_name, 
                           labels = c(expression('D = 0.06'~lambda[0]~'= 2'),
...
bias_plot = all_sims %>% 
  ...
  geom_errorbar(aes(ymax = ucl,ymin = lcl),
                position = position_dodge(width = .05), width = .01,
                show.legend = F) +
...
par_se_dat <- function(scr_fit,
  ...
  mod_real.se = rbind(scr_real_se,scr2_real_se) %>%
    ...
    group_by(mod,ghost_prop) %>%
    summarise(EN_mean.se = mean(EN_se), 
              EN_lcl.se = EN_mean.se - 1.96 * sd(EN_se)/sqrt(n()),
              EN_ucl.se = EN_mean.se + 1.96 * sd(EN_se)/sqrt(n()),
              lambda0_mean.se = mean(lambda0_se), 
              lambda0_lcl.se = lambda0_mean.se - 1.96 * sd(lambda0_se)/sqrt(n()),
              lambda0_ucl.se = lambda0_mean.se + 1.96 * sd(lambda0_se)/sqrt(n()),
              sigma_mean.se = mean(sigma_se), 
              sigma_lcl.se = sigma_mean.se - 1.96 * sd(sigma_se)/sqrt(n()),
              sigma_ucl.se = sigma_mean.se + 1.96 * sd(sigma_se)/sqrt(n())) %>%
...
se_plot <- all_sims %>% 
  ...
  scale_shape_discrete(name = 'Model') +
  geom_point(position = position_dodge(width = .05), size = 3, show.legend = T) +
  geom_errorbar(aes(ymax = ucl.se,ymin = lcl.se),
...
ggarrange(bias_plot,se_plot, log_mse_plot,nrow = 3, common.legend = T, 
...
sim_capt_summaries <- function(pop, capthist){
  ...
  data.frame(total_dets = total_dets,
             individuals_dets = individuals_dets,
